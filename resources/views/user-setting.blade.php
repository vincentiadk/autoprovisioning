        ...
        <div class="col-md-12">
            <form action="{{ url('panel/user/store') }}" method="post" enctype="multipart/form-data" id="form_data">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                        @if($data['nwuser_decrypted'] != "")
                        <span id="tacacs_status" class="alert @if(str_contains($data['tacacs_notification'], 'not')) alert-danger @else alert-success @endif">{{$data['tacacs_notification'] }}</span>
                        @else 
                        <span id="tacacs_status" class="alert alert-danger">You have not specify username/password for TACACS IP</span>
                        @endif
                        </div>
                        <div class="form-group">
                            <label>USER</label>
        ...
        success: function(response) {
            ...
            if (response.status == 200) {
                if (id_ > 0) {
                    ...
                    $('#tacacs_status').text(response.tacacs_notification);
                    if(response.tacacs_valid == "1" || response.tacacs_valid == "true" ){
                        $('#tacacs_status').removeClass("alert-danger").addClass("alert-success");
                    } else {
                        $('#tacacs_status').removeClass("alert-success").addClass("alert-danger");
                    }
                } else {
